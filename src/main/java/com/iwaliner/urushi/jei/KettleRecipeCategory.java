...
public class KettleRecipeCategory implements IRecipeCategory<KettleRecipe> {
    ...
    public KettleRecipeCategory(IGuiHelper guiHelper){
        this.background=guiHelper.createDrawable(tex,55,16,82,54);
        this.icon=guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK,new ItemStack(ItemAndBlockRegister.kettle.get()));
    ...
    public RecipeType<KettleRecipe> getRecipeType() {
        return JEIUrushiPlugin.JEI_KETTLE;
    }

    @Override
    public Component getTitle() {
        return Component.translatable("gui.jei.category.urushi.kettle");
    }
    ...
    protected void drawCookTime(KettleRecipe recipe, GuiGraphics guiGraphics, int y) {
        int cookTime = 100;
        if (cookTime > 0) {
    ...
    protected IDrawableAnimated getArrow(KettleRecipe recipe) {
        int cookTime = 100;
        if (cookTime <= 0) {
